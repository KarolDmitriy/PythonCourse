...
-- Создание процедуры для перемещения пользователя в users_old
DELIMITER //
CREATE PROCEDURE MoveUserToOld(IN userId BIGINT UNSIGNED)
BEGIN
    ...
    IF userCount > 0 THEN
        ...
        COMMIT;
        SELECT 'User moved successfully' AS result;
    ELSE
        ROLLBACK;
...
-- Создание триггеров для users, communities и messages
DELIMITER //
...
CREATE TRIGGER Communities_Insert_Log AFTER INSERT ON communities
FOR EACH ROW
BEGIN
    INSERT INTO logs (table_name, record_id) VALUES ('communities', NEW.id);
END //

CREATE TRIGGER Messages_Insert_Log AFTER INSERT ON messages
FOR EACH ROW
BEGIN
